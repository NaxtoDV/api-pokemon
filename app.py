...
async def fetch_and_save_pokemon(search_term: str):
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="PokeAPI tardó demasiado en responder")
    except httpx.ConnectError:
        raise HTTPException(status_code=503, detail="No se pudo conectar a PokeAPI")
...
async def search_pokemon(
    query: str = Query(
        ...
        min_length=1,
        max_length=100,
        pattern=r"^[a-zA-Z\-]+$",  # solo letras y guiones, bloquea números y caracteres especiales
...
async def get_pokemon_by_id(
    id: int = Path(
        ...,
        ge=1,       # mayor o igual a 1, no existe pokemon con id 0 o negativo
        le=100000,  # límite superior razonable
        description="ID numérico del Pokemon a buscar"
    )
):
    search_term = str(id)
    return await fetch_and_save_pokemon(search_term)
